...
extern "C" void (*irq_table[16])() = {irq0,  irq1,  irq2,  irq3, irq4,  irq5,
...
extern "C" void isr_handler(interrupt_frame *frame) {
    ...
    } else if (frame->vector == 13) // general protection fault
    {
        ...
        global_idt->kshell->print_kernel_error(
            ...
            frame->rip, frame->error, frame->error & 0xFFF8,
            (frame->error & 1) ? "yes" : "no",
            (frame->error & 2) ? "yes" : "no");
        serial_print("[FATAL] GPF - halting\n");
        while (true)
            asm volatile("hlt");
    } else if (frame->vector == 14) // page fault
    {
        ...
        serial_print("\n");
        global_idt->kshell->print_kernel_error(
            "Page fault at RIP=%x, fault addr=%x, error=%x%s%s%s", frame->rip,
            cr2, frame->error,
            (frame->error & 1) ? " (protection)" : " (not-present)",
            (frame->error & 2) ? " write" : " read",
            (frame->error & 4) ? " user" : " supervisor");
        serial_print("[FATAL] Page fault - halting\n");
        while (true)
            asm volatile("hlt");
        ...
        *(volatile uint32_t *)0xFEE000B0 = 0;
        ...
        serial_print("\n");
        global_idt->kshell->print_kernel_error(
            "Unhandled exception vector %d at RIP=%x, error=%x", frame->vector,
...
// Sends EOI to the local APIC (the PIC is disabled; we use the I/O APIC).
extern "C" void irq_handler(interrupt_frame *frame) {
    ...
    // Ack the interrupt source to prevent re-assertion.
    // For IRQ1 (keyboard), read the PS/2 data port to clear the output buffer.
    if (irq == 1)
        inb(0x60);

    // Send EOI to the local APIC (LAPIC EOI register at 0xFEE000B0)
    *(volatile uint32_t *)0xFEE000B0 = 0;
}
