...
void KShell::draw_char(char c)
{
    ...
    for (uint64_t row = 0; row < char_height; row++) {
        ...
        for (uint64_t col = 0; col < char_width; col++) {
            if (bits & (1 << (7 - col))) {
                graphics->draw_pixel(foreground_color, cursor_pos_x + col, cursor_pos_y + row);
            }
        }
    }
...
void KShell::backspace()
{
    if (cursor_pos_x == 0)
    {
        if (cursor_pos_y < char_height) return;
        cursor_pos_y -= char_height;
    ...
    for (uint64_t y = 0; y < char_height; y++)
    {
        ...
        {
            graphics->draw_pixel(background_color, cursor_pos_x + x, cursor_pos_y + y);
        }
    }
}
    ...
    for (int i = 0; fmt[i] != '\0'; i++) 
    {
        if (fmt[i] == '%') 
        {
            ...
            switch (fmt[i]) 
            {
                ...
                case 'd': {
                    ...
                    for (int j = idx-1; j >= 0; j--) draw_char(buf[j]);
                    break;
                }
                case 'u': 
                {
                ...
                {
                    unsigned int val = va_arg(args, unsigned int);
                    for(int j=31;j>=0;j--) draw_char((val & (1<<j)) ? '1' : '0');
                    break;
                }
                case 'c': 
                {
        ...
        else if(fmt[i]=='\t')
        {
            ...
            if(cursor_pos_x >= max_width - safeguard_width)
            {
                cursor_pos_x = 0;
                cursor_pos_y += char_height;
                if(cursor_pos_y >= max_height - safeguard_height) scroll();
            }
        } 
        else 
        {
